...
- ⚠️ Предупредить о рисках и ограничениях
...
Самый простой способ запустить весь проект с визуализацией:

```bash
# Откройте файл LAUNCHER.html в браузере
# или запустите автоматический скрипт:
./start-project.sh
```

**LAUNCHER.html** — это интерактивная страница запуска, которая:
- ✓ Проверяет статус приложения в реальном времени
- ✓ Показывает инструкции по запуску
- ✓ Автоматически перенаправляет на приложение когда оно готово
- ✓ Содержит ссылки на все части проекта (Frontend, API Docs, Rules API)

**start-project.sh** — это shell-скрипт, который:
- ✓ Проверяет наличие Docker и Docker Compose
- ✓ Автоматически запускает все сервисы
...
**Сделано с ❤️ для упрощения жизни миллионов людей**